// The playground. Every call below is synchronous WebAssembly: there is no
// server behind this page and no network traffic after the engine loads.
//
// This is the repository's `demo/demo.js` with three changes, and it is a copy
// rather than an import because the two have different jobs -- `demo/` is a
// verification harness that must run from a bare `python3 -m http.server` in a
// checkout, and this is a page of a built site. Keeping them separate means
// neither constrains the other.
//
// The import is relative on purpose. The site is deployed under a path prefix
// (`/accent-proust` on GitHub Pages), and a module specifier resolved against
// this file's own URL is the one form that needs no knowledge of that prefix --
// including inside the wasm-bindgen glue, which fetches its `.wasm` sibling the
// same way.
import init, { renderHtml, validate, transform, format } from "../wasm/accent_proust_wasm.js";
...
// page would restyle the site's own chrome; and a sandbox with no
...
function frame(html) {
  const t = theme();
  const link = t.href ? `<link rel="stylesheet" href="${t.href}">` : "";
...
function run() {
  ...
  timing.textContent = `${kb} KB parsed, validated, transformed and rendered in ${took}`;
}
...
// Not in the repository's demo. A visitor who has edited the sample into
// nothing has no way back to the thing the page exists to show; a developer
// running the demo from a checkout can just reload.
...
try {
  ...
  // On the deployed site this means the build shipped without the engine, so
  // the actionable half is a bug report rather than a build command.
  status.textContent = "The engine did not load";
  status.classList.add("bad");
  source.value = SAMPLE;
  source.disabled = true;
  timing.textContent =
    "The WebAssembly module failed to instantiate. The console has the reason.";
  console.error(error);
}
